// app/api/campaigns/[campaignId]/report-config/route.ts
...
export async function GET(req: Request, ctx: { params: Promise<{ campaignId: string }> }) {
  try {
    ...
    const upstream = await fetch(`${BASE}/campaigns/${campaignId}/report-config`, {
      method: "GET",
...
export async function PUT(req: Request, ctx: { params: Promise<{ campaignId: string }> }) {
  try {
    if (!BASE) return NextResponse.json({ error: "Missing IMPORT_API_BASE_URL" }, { status: 500 })
    const auth = passthroughAuth(req)
    const { campaignId } = await ctx.params
